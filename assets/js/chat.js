...
signaling_socket.on('newTextMsg', function (data) {
    ...
    if (data.queryId == queryLink) {
        ...
        document.getElementById('message-container').innerHTML += '<div class="direct-chat-info clearfix"><span class="direct-chat-name pull-left">'
            + data.userName + '</span></div><i class="fa fa-user-circle fa-2x" style="margin-left: 3px; margin-top: 4px;"></i><div class="content direct-chat-text new_windowAutoLink">' + data.message + '</div><div class="direct-chat-info clearfix"><span class="direct-chat-timestamp pull-right">' + time + '</span></div>'
        autoLinkNeed();
        scrollDown();
